...
export function useIdentity() {
  ...
  const coreRef = useRef<typeof import("commonchat-core") | null>(null);

  const loadStored = useCallback(async () => {
    try {
      const core = await import("commonchat-core");
  ...
  const initializeWithDisplayName = useCallback(
    async (name: string) => {
      ...
      if (!core) {
        const mod = await import("commonchat-core");
        coreRef.current = mod;
  ...
  const verifySignature = useCallback(
    async (pubKeyHex: string, message: string, signatureHex: string): Promise<boolean> => {
      try {
        const c = coreRef.current ?? (await import("commonchat-core"));
